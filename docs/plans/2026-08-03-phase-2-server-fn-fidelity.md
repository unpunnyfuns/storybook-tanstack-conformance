...
- **A branch that changes the type of anything exported must also pass a real package build**, not just `--noEmit`. `--noEmit` skips declaration emit, so an entire class of error (`TS2883` and friends, where an inferred type cannot be named portably in a `.d.ts`) is invisible to it. `fix/tanstack-create-start-instance` shipped through review with a build that fails. Run it from the repo root, and note that `yarn build` needs `node_modules/.bin` on PATH in this shell: `cd /Users/palnes/src/sbfork/scripts && PATH="/Users/palnes/src/sbfork/node_modules/.bin:$PATH" NODE_ENV=production jiti ./build-package.ts tanstack-react --no-watch --prod`.
...
These are defects of the composition. None exists on any single branch, so none can be fixed before the merge, and the branch-local reviews could not have found them.

- **CRITICAL. `createMiddleware(...).validator()` becomes a hard `TypeError` under the opt-in.** The real `createMiddleware` has no `.validator` method: the builder's keys are exactly `options, middleware, inputValidator, client, server`. It only appears to work today because phase 1's `fix/tanstack-strip-validator` deletes the call at build time, which its own test asserts. `feat/tanstack-execute-server-fns` rewrote that same conditional to suspend the `createMiddleware` strip under the opt-in, and the two branches conflict on it. The resolution both branches' comments invite, gating all three method names on `!executeServerFunctions`, lets the call survive into the browser, where it throws `createMiddleware(...).validator is not a function` **at module evaluation**, taking down the whole story file rather than one server function. The safe resolution is to keep `validator` outside the negation so it is always stripped, accepting that `.validator` and `.inputValidator` then behave differently on the same builder. No conformance app writes `createMiddleware().validator()`, so **the suite will not catch this**. Decide it deliberately at merge time.
- **The `createStart` conflict has no correct git resolution.** `fix/tanstack-create-start-instance` returns `{ getOptions, createMiddleware }`; `fix/tanstack-server-fn-delegation` returns `{}` and instead publishes `__TSS_START_OPTIONS__` and warns about global middleware. Either side alone loses the other's entire point, and the union is also wrong: the delegation branch calls `getOptions()` eagerly at construction while the real `createStart` is entirely lazy and only calls the thunk inside `getOptions()`. Composed naively, a user's config thunk runs once eagerly at module evaluation and again on every `getOptions()`. Nothing tests the composed shape; write that test as part of the merge.
- **A `@vitest-environment` pragma collision reclassifies about 300 lines of tests and breaks one.** `fix/tanstack-use-server-fn-redirect` puts `// @vitest-environment happy-dom` on line 1 of `export-mocks/start.test.ts`; the delegation and cookie branches add large Node-environment bodies to the same file. Git puts the pragma inside the conflict hunk, and the only resolution that keeps both bodies flips the whole file to happy-dom, where the cookie branch's request-scope test fails. Split the file, or move the DOM tests into their own.
- **`getCookie()` is unconditionally `undefined` in a real browser after `fix/tanstack-cookie-scope`, and that is a design gap rather than a merge artifact.** The only writer of the request-cookie map is `parseCookieHeader(request.headers.get('cookie'))`, and `setCookie` correctly no longer writes it. But `cookie` is on the fetch spec's forbidden-header list, so a browser silently drops it and the header can never be present. The branch's unit test passes only because vitest's default Node environment permits the forbidden header. After this branch a story has **no way to seed a request cookie at all**, which needs a deliberate answer: either a story parameter that seeds the request scope, in the manner of `parameters.tanstack.start.context`, or an explicit documented statement that request cookies are unreadable in stories. The `Cookie Scope` gauge still goes green, but for a wider reason than it documents.

### Gauge verdicts for a composed `patched` build

Probed against the composed source rather than predicted. Four of the five server-probe gauges go green, plus the new reset gauge. **`ValidatorTransforms` stays red**, for the already-recorded ungated-strip reason: with the opt-in on, the eliminator keeps the handler and the middleware `server` phase but still deletes `.validator(Number)`, so `increment({data: "1"})` concatenates instead of adding. Fixing the validator gate above is what flips it.

### A near-miss worth keeping

`createFallbackStartContext()` returns `undefined` unless `__TSR_ROUTER__` or `__TSS_START_OPTIONS__` is set, and `__executeServer` dereferences the context without `throwIfNotFound: false`. Every delegation-branch unit test manufactures that precondition with a `createStart()` call in `beforeEach`, and nothing in a real Storybook preview calls `createStart`. The path is nonetheless safe in a browser, because `router-core`'s Router constructor sets `self.__TSR_ROUTER__` whenever `document` exists and the decorator builds a real router for every story. So the guarantee holds only in a browser, and **no unit test exercises the path a story actually takes**. If those tests ever move to a non-DOM runner, or the `createStart` crutch is removed, the failure mode is a hard throw on every server function.
